...
const TransactionModal = ({ isOpen, onClose, type, onSuccess }) => {
  ...
  return (
    ...
    <div className="fixed inset-0 bg-black/80 flex justify-center items-center z-50 backdrop-blur-sm">
      ...
      {/*  Modal */}
      <div className={`bg-gray-800 p-8 rounded-2xl w-96 border-2 ${type === 'income' ? 'border-emerald-500' : 'border-red-500'} shadow-2xl`}>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div>
            ...
            <input 
              ...
              className="w-full bg-gray-900 text-white border border-gray-700 rounded-lg p-3 focus:outline-none focus:border-blue-500 transition-colors"
              value={description}
          ...
          <div className="flex gap-3 mt-4">
            ...
            <button 
              type="submit" 
              className={`flex-1 py-3 rounded-xl font-bold text-white transition-all ${type === 'income' ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-red-600 hover:bg-red-700'}`}
            >
              Salvar
            </button>
          </div>
